...
⚠️ Status do projeto: em desenvolvimento O Beebop está atualmente na fase de construção do MVP (Produto Mínimo Viável). Novas funcionalidades, páginas e ajustes visuais ainda estão sendo implementados. Consulte a seção Status atual para saber o que já está disponível.

📌 Objetivo

Construir um MVP funcional, 100% client-side, que consuma dados da iTunes Search API para busca, filtragem, ordenação e exibição de detalhes de músicas, artistas e álbuns.
...
bash
   cd Beebop
Abra o arquivo index.html diretamente no navegador ou utilize uma extensão como o Live Server no VS Code para uma melhor experiência de desenvolvimento (recarregamento automático).
